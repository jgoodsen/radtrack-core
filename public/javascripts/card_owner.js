CardOwnerWidget = {
  ...
  display: function(card, users) {
    return CardOwnerWidget.card_owner(card) + CardOwnerWidget.owner_select_list(card, users) + CardOwnerWidget.done_button();
  },
  ...
  setup_events: function(parent){
    ...
    $(".card_owner_select", parent).change(function(){
      $.ajax({
        type: "POST",
        url: "http://" + DOMAIN_AND_PORT + "/cards/" + ElementId.card_id($(this)),
        data: "_method=put&card[user_id]=" + $(this).val() + 
                "&project_id=" + ElementId.extract_project_id($(this)) + 
